...
默认 SQLite 数据库路径为 `~/.t-build/t-build.sqlite`。如果需要自定义数据位置，可以设置 `T_BUILD_DATA_DIR`、`T_BUILD_DATABASE_PATH`，或使用 `DATABASE_URL=file:/absolute/path/t-build.sqlite`。请不要提交 `.env.local`。
